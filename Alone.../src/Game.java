...
public class Game {
    ...
    public void init() {
        try {
            this.font = Font.createFont(Font.TRUETYPE_FONT, new File("./src/font.ttf"));
        ...
        try {
            ...
            gameMusic[0].open(AudioSystem.getAudioInputStream(new File("./src/audio/music/MAIN_MENU.wav")));
        } catch (Exception ex) {
        ...
        for (int i = 1; i < gameMusic.length; i++) {
            try {
                ...
                gameMusic[i].open(AudioSystem.getAudioInputStream(new File("./src/audio/music/" + GameModes.values()[i] + ".wav")));
            } catch (Exception ex) {
                Main.errorScreen("ERROR LOADING MUSIC", "PLEASE ENSURE 2 WAV FILES EXISTS IN THE AUDIO/MUSIC/ FOLDER");
            }
        }
        for (int i = 0; i < soundEffects.length; i++) {
            try {
                ...
                soundEffects[i].open(AudioSystem.getAudioInputStream(new File("./src/audio/soundEffects/" + i +".wav")));
            } catch (Exception ex) {
